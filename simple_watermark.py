...
def add_watermark(image_path, output_path, name, border_color, text_color):
    ...
    position_bottom_right = (img_width - box_width - 10, img_height - box_height - 10)
    border_width = 2
    draw.rectangle([position[0] - border_width, position[1] - border_width,
                    ...
                    width=border_width) # only border
    ...
    if len(date_string) > 4:
        ...
        date_string = subprocess.check_output(['exiftool', '-FileModifyDate', image_path]).decode()
        date_string = date_string[date_string.index(":")+1:].strip()
        datee = date_string.split()[0]
        timee = re.search(r"[0-9]+:[0-9]+:[0-9]+", date_string.split()[1])[0].split(':')
        d, m, y, H, M = datee[2], datee[1], datee[0], timee[0], timee[1]
        date = f"{y}/{m}/{d} - {H}:{M}"
        # print(date)
    
    font_size = 30 #40 # (img_width**2 + img_height**2) //3033
    ...
    date_text = f"{' '*padding_date}{'other_text'}{' '*(padding_date-1)}\n"#f"{' '*padding_date}{date}{' '*(padding_date-1)}\n"
...
if __name__ == "__main__":
    ...
    for img in tqdm(images):
        add_watermark(dest_images + img, dest_images + img, 'Your name', "blue", "blue")
